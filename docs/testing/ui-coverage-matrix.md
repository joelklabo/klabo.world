...
| --- | --- | --- | --- | --- |
| Home page | Hero content, latest posts/apps, tag cloud, footer links | ✅ (playwright `home.spec.ts`) | Responsive snapshots (mobile/desktop) | Needs ≥3 posts, ≥1 app |
...
| Search page | `/search` combined results, keyboard navigation | ⚠️ only smoke coverage | Manual: search ≤1 char blocked, highlight terms | Requires posts/apps data |
...
| Markdown preview | `/admin/markdown-preview` for posts/apps | ✅ unit (Vitest) only | Manual: compare preview vs. published | Down/markdown fixtures |
| Public APIs | `/rss.xml`, `/feed.json`, `/api/search` | ⚠️ only unit tests | Manual: curl APIs in staging | Content data |
