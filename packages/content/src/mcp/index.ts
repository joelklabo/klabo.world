...
import { uploadImage, type ImageConfig } from '../core/images/index.js';
...
export const toolDefinitions = [
  ...
  // Annotation tools
  {
    ...
    description:
      'List all annotations for a draft with status counts. Returns open, resolved, and archived counts.',
    inputSchema: {
      type: 'object',
      properties: {
        draftSlug: {
  ...
  },
  {
    ...
    description:
      'Create a new annotation on a draft. Claude can use this to annotate content for the user.',
    inputSchema: {
      type: 'object',
      properties: {
        draftSlug: {
...
async function handleTool(name: string, input: ToolInput): Promise<ToolResult> {
  ...
  try {
    switch (name) {
      ...
      case 'kw_draft_update': {
        const partialInput: Partial<DraftInput> = {};
        if (input.title) partialInput.title = input.title as string;
        if (input.summary) partialInput.summary = input.summary as string;
        if (input.body) partialInput.body = input.body as string;
        if (input.tags) partialInput.tags = input.tags as string[];
        if (input.featuredImage) partialInput.featuredImage = input.featuredImage as string;
      ...
      case 'kw_draft_get': {
        const result = await getDraft(input.slug as string, storage, draftConfig);
        if (!result) {
          return {
            content: [
              {
      ...
      case 'kw_draft_delete': {
        const result = await deleteDraft(input.slug as string, storage, draftConfig);
        return {
          content: [
            {
              type: 'text',
      ...
      case 'kw_draft_publish': {
        const result = await publishDraft(input.slug as string, storage, draftConfig, {
          publishDate: input.publishDate as string | undefined,
        });
        return {
          content: [
            {
      ...
      case 'kw_image_upload': {
        ...
        const result = await uploadImage(
          { file: buffer, filename, mimeType, altText },
          imageConfig
        );
        return {
          content: [
      ...
      case 'kw_annotation_list': {
        ...
        try {
          const result = await listAnnotations(
            input.draftSlug as string,
            db,
            input.status ? { status: input.status as AnnotationStatus } : undefined
          );
          return {
            content: [
              {
      ...
      case 'kw_annotation_get': {
        const db = createAnnotationAdapter();
        try {
          const result = await getAnnotation(input.id as string, db);
          if (!result) {
            return {
              content: [
                {
      ...
      case 'kw_annotation_create': {
        ...
        try {
          const annotationInput: AnnotationInput = {
            draftSlug: input.draftSlug as string,
            type: input.type as AnnotationInput['type'],
            content: input.content as string,
            selectors: input.selectors as AnnotationInput['selectors'],
      ...
      case 'kw_annotation_update': {
        const db = createAnnotationAdapter();
        try {
          const updateInput: AnnotationUpdate = {};
          if (input.content) updateInput.content = input.content as string;
          if (input.status) updateInput.status = input.status as AnnotationUpdate['status'];
          if (input.color) updateInput.color = input.color as string;

          const result = await updateAnnotation(input.id as string, updateInput, db);
          return {
            content: [
              {
                type: 'text',
      ...
      case 'kw_annotation_resolve': {
        const db = createAnnotationAdapter();
        try {
          const result = await resolveAnnotation(input.id as string, db);
          return {
            content: [
              {
                type: 'text',
      ...
      case 'kw_annotation_delete': {
        const db = createAnnotationAdapter();
        try {
          const result = await deleteAnnotation(input.id as string, db);
          return {
            content: [
              {
                type: 'text',
