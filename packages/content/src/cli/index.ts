...
import { uploadImageFromPath, type ImageConfig } from '../core/images/index.js';
import { LocalStorageProvider } from '../core/storage/local.js';

// Resolve project paths
function resolveConfig(): { draft: DraftConfig; image: ImageConfig; storage: LocalStorageProvider } {
...
function output<T>(response: ApiResponse<T>): void {
  console.log(JSON.stringify(response, null, 2));
}

// Build next actions for create
function createNextActions(slug: string): NextAction[] {
  return [
...
function publishNextActions(slug: string): NextAction[] {
  return [
  ...
  draft
    ...
    .action(async (options) => {
      try {
        ...
        const uploadedImages: Array<{ source: string; url: string; markdown: string }> = [];
        if (options.images) {
  ...
  draft
    ...
    .action(async (slug, options) => {
      try {
        ...
        if (options.summary) input.summary = options.summary;
        if (options.body) input.body = await fs.readFile(options.body, 'utf-8');
        if (options.tags) input.tags = options.tags.split(',').map((t: string) => t.trim());
        if (options.featuredImage) input.featuredImage = options.featuredImage;
