...
interface WatcherState {
  lastCheck: Date;
  knownAnnotations: Map<string, { updatedAt: Date; status: string }>;
}
...
function broadcast(changes: AnnotationChange[]) {
...
// Poll for changes
async function pollForChanges(db: SqliteAnnotationAdapter): Promise<AnnotationChange[]> {
  ...
  try {
    ...
    for (const annotation of annotations) {
      ...
      if (!known) {
        ...
        annotation.updatedAt.getTime() > known.updatedAt.getTime() ||
...
async function initializeState(db: SqliteAnnotationAdapter) {
  try {
    const annotations = await db.findMany({
      where: {},
      orderBy: { createdAt: 'asc' },
