...
export type ImageConfig = {
  uploadsDir: string; // e.g., /path/to/public/uploads
...
export function buildImageMarkdown(url: string, altText?: string): string {
  const alt = altText || '';
  return `![${alt}](${url})`;
}
...
export async function uploadImage(
  ...
  if (!SUPPORTED_TYPES.has(input.mimeType)) {
    throw new Error(
      `Unsupported image type: ${input.mimeType}. Supported: ${[...SUPPORTED_TYPES].join(', ')}`
    );
  }
  ...
  if (input.file.length > MAX_SIZE) {
    throw new Error(
      `File too large: ${(input.file.length / 1024 / 1024).toFixed(2)}MB. Max: 10MB`
    );
  }
...
export async function listImages(config: ImageConfig): Promise<string[]> {
  try {
