...
// Database adapter interface for Prisma
export interface AnnotationDbAdapter {
  ...
  findUnique(args: { where: { id: string }; include?: { replies?: boolean } }): Promise<
    (AnnotationDbRecord & { replies?: AnnotationDbRecord[] }) | null
  >;

  create(args: { data: AnnotationDbCreateInput }): Promise<AnnotationDbRecord>;
  ...
  delete(args: { where: { id: string } }): Promise<AnnotationDbRecord>;

  deleteMany(args: { where: { parentId: string } }): Promise<{ count: number }>;

  count(args: { where: { draftSlug: string; status?: string } }): Promise<number>;
...
function toAnnotationMetadata(record: AnnotationDbRecord): AnnotationMetadata {
  return {
    id: record.id,
    draftSlug: record.draftSlug,
    type: record.type as AnnotationMetadata['type'],
    status: record.status as AnnotationMetadata['status'],
    content: record.content,
    selectors: JSON.parse(record.selectors) as Selector[],
    color: record.color,
...
export async function updateAnnotation(
  ...
  return {
    id: record.id,
    status: record.status as AnnotationStatus,
  };
}
...
export async function resolveAnnotation(
  id: string,
  db: AnnotationDbAdapter
): Promise<ResolveAnnotationResult> {
...
export async function archiveOrphanedAnnotations(
  draftSlug: string,
  orphanedIds: string[],
  db: AnnotationDbAdapter
