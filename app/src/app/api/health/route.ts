...
export async function GET() {
  ...
  if (missing(env.APPLICATIONINSIGHTS_CONNECTION_STRING)) {
    missingSecrets.push('APPLICATIONINSIGHTS_CONNECTION_STRING');
  }
  const hasLogAnalytics =
    !missing(env.LOG_ANALYTICS_WORKSPACE_ID) && !missing(env.LOG_ANALYTICS_SHARED_KEY);
  const hasAppInsightsApi =
    !missing(env.APPINSIGHTS_APP_ID) && !missing(env.APPINSIGHTS_API_KEY);
  ...
  const hasGistToken = !missing(env.GITHUB_TOKEN);
