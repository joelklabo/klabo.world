...
- **Commit workflow:** Always use the globally installed `commit-push-watch.sh` helper so CI is tracked automatically even when you detach and keep working.
  ...
  - **Every issue closure must be paired with its own commit and push**—close only after that commit exists so beads history, git history, and CI runs match.
...
NOSTRSTACK_BASE_URL=
NOSTRSTACK_HOST=
NOSTRSTACK_LN_ADDRESS=
NOSTRSTACK_NOSTR_PUBKEY=
NOSTRSTACK_RELAYS=
```
