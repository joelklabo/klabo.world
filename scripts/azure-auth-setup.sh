...
# Run the hasher
hash=$("$SWIFT_CMD" run --skip-build KlaboWorld eval "import Vapor; let app = Application(); defer { app.shutdown() }; print(try app.password.hash(\"$password\"))" 2>/dev/null | tail -n 1)

if [ -z "$hash" ]; then
    echo "Error: Failed to generate password hash"
    echo "Running fallback method..."
    hash=$(echo "$password" | "$SWIFT_CMD" run KlaboWorld hash-password 2>/dev/null | grep '^\$2b' | head -n 1)
fi
